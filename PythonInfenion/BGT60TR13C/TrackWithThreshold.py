...
#def run_range_angle_map(detection_threshold_db=-20):
    ...
    with DeviceFmcw() as device:
        ...
        while not plot.is_closed():
            try:
                ...
                # Convert to dB scale
                beam_range_energy_db = 20 * np.log10(beam_range_energy)

                # Apply threshold
                # eam_range_energy_db_thresholded = np.where(beam_range_energy_db > detection_threshold_db, 
                #                                             beam_range_energy_db, 
                #                                             np.nan)


                # Find maximum above threshold
                # max_value = np.nanmax(beam_range_energy_db_thresholded)

                # if np.isfinite(max_value):
                #     max_row, max_col = np.unravel_index(np.nanargmax(beam_range_energy_db_thresholded), 
                #                                         beam_range_energy_db_thresholded.shape)
                ...
                #               f"Detected entity: angle={angle_degrees:+02.0f}°, range={range_m:.2f}m, strength={max_value:.2f}dB")
                # else:
                #     plot.draw(None, None, "No detection above threshold")
...
if __name__ == "__main__":
    #run_range_angle_map(detection_threshold_db=-20)  # You can adjust this threshold
    run_range_angle_map()